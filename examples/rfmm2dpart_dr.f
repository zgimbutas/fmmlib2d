...
c
        ...
        real *8 charge1(2 000 000)
        real *8 dipstr1(2 000 000)
        complex *16 charge(2 000 000)
        complex *16 dipstr(2 000 000)
        real *8 dipvec(2,2 000 000)
        complex *16 pot(2 000 000)
        complex *16 grad(2 000 000)
        complex *16 hess(2 000 000)
        complex *16 pottarg(2 000 000)
        complex *16 gradtarg(2 000 000)
        complex *16 hesstarg(2 000 000)
c       
        real *8 pot1(2 000 000)
        real *8 grad1(2,2 000 000)
        real *8 hess1(3,2 000 000)
        real *8 pot2(2 000 000)
        real *8 grad2(2,2 000 000)
        real *8 hess2(3,2 000 000)

        real *8 target(2,2 000 000)
        real *8 pottarg1(2 000 000)
        real *8 gradtarg1(2,2 000 000)
        real *8 hesstarg1(3,2 000 000)
        real *8 pottarg2(2 000 000)
        real *8 gradtarg2(2,2 000 000)
        real *8 hesstarg2(3,2 000 000)
c
...
c
        ...
        do i=1,nsource
        charge(i)=charge1(i)
        dipstr(i)=-dipstr1(i)*(dipvec(1,i)+ima*dipvec(2,i))
        enddo
...
c       
        do i=1,nsource
        pot1(i)=dble(pot(i))
        grad1(1,i)=dble(grad(i))
        grad1(2,i)=-imag(grad(i))
        hess1(1,i)=dble(hess(i))
        hess1(2,i)=-imag(hess(i))
        hess1(3,i)=-dble(hess(i))
        enddo
c
        do i=1,ntarget
        pottarg1(i)=dble(pottarg(i))
        gradtarg1(1,i)=dble(gradtarg(i))
        gradtarg1(2,i)=-imag(gradtarg(i))
        hesstarg1(1,i)=dble(hesstarg(i))
        hesstarg1(2,i)=-imag(hesstarg(i))
        hesstarg1(3,i)=-dble(hesstarg(i))
        enddo
        endif
...
c
     ...
     $     nsource,source,
     $     ifcharge,charge1,ifdipole,dipstr1,dipvec,
...
cccC$OMP$NUM_THREADS(4) 
     ...
     $     ifcharge,charge1(i),ifdipole,dipstr1(i),dipvec(1,i),
     1     source(1,j),ifpot,ptemp,ifgrad,gtemp,ifhess,htemp)
        if (ifpot .eq. 1) pot2(j)=pot2(j)+ptemp
        if (ifgrad .eq. 1) then
           grad2(1,j)=grad2(1,j)+gtemp(1)
           grad2(2,j)=grad2(2,j)+gtemp(2)
        endif
        if (ifhess .eq. 1) then
...
c
        if (ifprint .eq. 1) then
        if( ifpot.eq.1 ) call prin2('after fmm, pot1=*',pot1,m)
        if( ifgrad.eq.1 ) call prin2('after fmm, grad1=*',grad1,2*m)
        if( ifhess.eq.1 ) call prin2('after fmm, hess1=*',hess1,3*m)
        if( ifpot.eq.1 ) call prin2('directly, pot=*',pot2,m)
        if( ifgrad.eq.1 ) call prin2('directly, grad=*',grad2,2*m)
        if( ifhess.eq.1 ) call prin2('directly, hess=*',hess2,3*m)
        endif
c
...
c
     ...
     $     call prin2('after fmm, hesstarg=*',hesstarg,2*m)
        ...
        if (ifpottarg .eq. 1) 
     $     call prin2('after fmm, pottarg1=*',pottarg1,m)
        if( ifgradtarg.eq.1 ) 
     $     call prin2('after fmm, gradtarg1=*',gradtarg1,2*m)
        if( ifhesstarg.eq.1 ) 
     $     call prin2('after fmm, hesstarg1=*',hesstarg1,3*m)

        if (ifpottarg .eq. 1) 
     $     call prin2('directly, pottarg=*',pot2,m)
        if( ifgradtarg.eq.1 ) 
     $     call prin2('directly, gradtarg=*',grad2,2*m)
        if( ifhesstarg.eq.1 ) 
     $     call prin2('directly, hesstarg=*',hess2,3*m)
        endif
c
        call prin2('directly, estimated time (sec)=*',
...
c       
        ...
        call l2derror1(pottarg1,pot2,m,aerr,rerr)
ccc        call prin2('absolute L2 error in potential=*',aerr,1)
        call prin2('relative L2 error in target potential=*',rerr,1)
        endif
c
        if (ifgradtarg .eq. 1) then
        call l2derror1(gradtarg1,grad2,2*m,aerr,rerr)
ccc         call prin2('absolute L2 error in gradient=*',aerr,1)
        call prin2('relative L2 error in target gradient=*',rerr,1)
        endif
c       
        if (ifhesstarg .eq. 1) then
        call l2derror1(hesstarg1,hess2,3*m,aerr,rerr)
ccc         call prin2('absolute L2 error in hessian=*',aerr,1)
...
c
c
        subroutine l2derror1(pot1,pot2,n,ae,re)
        implicit real *8 (a-h,o-z)
c
c       evaluate absolute and relative errors of the real part
c
        real *8 pot1(n),pot2(n)
c
        d=0
        a=0
c       
