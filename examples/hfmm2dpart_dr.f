...
c       
        ...
        complex *16 grad2(2,2 000 000)
        complex *16 hess2(3,2 000 000)
c       
        real *8 target(2,2 000 000)
        complex *16 pottarg(2 000 000)
        complex *16 gradtarg(2,2 000 000)
        complex *16 hesstarg(3,2 000 000)
c
...
C$        t1=omp_get_wtime()
c       
        call hfmm2dparttarg(ier,iprec,zk,
     $     nsource,source,
...
c
        ...
        if( ifgrad.eq.1 ) call prin2('after fmm, grad=*',grad,2*2*m)
        if( ifhess.eq.1 ) call prin2('after fmm, hess=*',hess,2*3*m)
        endif
c
c
c
...
cccC$OMP$NUM_THREADS(4) 
        ...
        if( i .eq. j ) goto 7150
        if( ifcharge .eq. 1 ) then
        call hpotgrad2d(ifgrad,ifhess,source(1,i),charge(i),
     $     source(1,j),zk,
     1     ptemp,gtemp,htemp)
        ...
        if (ifdipole .eq. 1) then
           call hpotgrad2d_dp(ifgrad,ifhess,source(1,i),
     $     dipstr(i),dipvec(1,i),
     $     source(1,j),zk,ptemp,gtemp,htemp)
           if (ifpot .eq. 1) pot2(j)=pot2(j)+ptemp
           if (ifgrad .eq. 1) then
              grad2(1,j)=grad2(1,j)+gtemp(1)
              grad2(2,j)=grad2(2,j)+gtemp(2)
...
c
        ...
        if( ifgrad.eq.1 ) call prin2('directly, grad=*',grad2,2*2*m)
        if( ifhess.eq.1 ) call prin2('directly, hess=*',hess2,2*3*m)
        endif
c
...
cccC$OMP$NUM_THREADS(4) 
        ...
        do 8150 i=1,nsource        
        if( ifcharge .eq. 1 ) then
        call hpotgrad2d(ifgradtarg,ifhesstarg,source(1,i),charge(i),
     $     target(1,j),zk,
     1     ptemp,gtemp,htemp)
        ...
        if (ifdipole .eq. 1) then
           call hpotgrad2d_dp(ifgradtarg,ifhesstarg,source(1,i),
     $     dipstr(i),dipvec(1,i),
     $     target(1,j),zk,ptemp,gtemp,htemp)
           if (ifpottarg .eq. 1) pot2(j)=pot2(j)+ptemp
           if (ifgradtarg .eq. 1) then
              grad2(1,j)=grad2(1,j)+gtemp(1)
              grad2(2,j)=grad2(2,j)+gtemp(2)
...
c
     ...
     $     call prin2('after fmm, pottarg=*',pottarg,2*m)
        if( ifgradtarg.eq.1 ) 
     $     call prin2('after fmm, gradtarg=*',gradtarg,2*2*m)
        if( ifhesstarg.eq.1 ) 
     $     call prin2('after fmm, hesstarg=*',hesstarg,2*3*m)
        if (ifpottarg .eq. 1) 
     $     call prin2('directly, pottarg=*',pot2,2*m)
        if( ifgradtarg.eq.1 ) 
     $     call prin2('directly, gradtarg=*',grad2,2*2*m)
        if( ifhesstarg.eq.1 ) 
     $     call prin2('directly, hesstarg=*',hess2,2*3*m)
        endif
c
        call prin2('directly, estimated time (sec)=*',
...
c       
        ...
        call h2derror(pottarg,pot2,m,aerr,rerr)
ccc        call prin2('absolute L2 error in potential=*',aerr,1)
        call prin2('relative L2 error in target potential=*',rerr,1)
        endif
c
        if (ifgradtarg .eq. 1) then
        call h2derror(gradtarg,grad2,2*m,aerr,rerr)
ccc         call prin2('absolute L2 error in gradient=*',aerr,1)
        call prin2('relative L2 error in target gradient=*',rerr,1)
        endif
c       
        if (ifhesstarg .eq. 1) then
        call h2derror(hesstarg,hess2,3*m,aerr,rerr)
ccc         call prin2('absolute L2 error in hessian=*',aerr,1)
...
c
c
        subroutine h2dmperr(mpole1,mpole2,nterms,d)
        implicit real *8 (a-h,o-z)
c       
...
c
c
        subroutine h2dmpnorm(mpole,nterms,d)
        implicit real *8 (a-h,o-z)
c
...
c
c
        subroutine h2derror(pot1,pot2,n,ae,re)
        implicit real *8 (a-h,o-z)
c
